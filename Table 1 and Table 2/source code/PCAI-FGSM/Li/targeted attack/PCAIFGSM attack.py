"""
The main function of this code file is to use the PCAI-FGSM to perform targeted attacks and generate adversarial samples under the Li norm constraint.
First, you need to set the "folder_path" to the absolute path of the folder containing the original images.
Second, you need to set the "save_path_for_adversarial_samples" to the absolute path of the folder where the corresponding adversarial samples will be stored.
Third, you need to set the "weights_path" to the weight file of the officially trained model, and the download link can be found in the paper.
Fourth, you need to set the "json_absolute_path" to the absolute path of the index file containing 1000 categories. This index file is named "output_class_indices.json" in the current directory.
Fifth, use from torchvision.models import ... to import the official convolutional neural network, and specify it using model_current.
Sixth, control the number of tampered pixels by setting the value of 'degree'.
Finally, executing this code file will generate the adversarial samples.
"""
...
for file_name in file_list:
    ...
    attack_index = target_index_image_path(image_path, json_absolute_path, weights_path, model_current)

    atk = PCAIFGSM(model, alpha=0.001, steps=max_num_iterative)
    # Set to targeted attack mode
    atk.set_mode_targeted_by_label()
    adv_image = atk(image, label, actual_image_transform_matrix, actual_image_matrix, actual_image)
    ...
    attack_image_index, x, _ = predict_image_path(iterative_image_path, json_absolute_path, weights_path, attack_index, model_current)
    ...
    actual_image_top1_index, x, actual_image_top500_index = predict_image_path(image_path, json_absolute_path,
    ...
    adversarial_sample_image_top1_index, x, _ = predict_image_path(iterative_image_path, json_absolute_path,
                                                                   weights_path, 0, model_current)
    ...
    while (right - left != 1):
        ...
        # Create PIL image object
        image_pil = Image.fromarray(image_rgb)
        image_pil.save("target.png")
        iterative_image_path = "target.png"

        iterative_image_current_top1_label, x, _ = predict_image_path(iterative_image_path, json_absolute_path,
                                                                      weights_path, 0, model_current)
